const rawText = `Q1. Find the features characteristic of mitosis:
...
const blocks = rawText.split(/Q\d+\.\s+/).filter(b => b.trim());
const questions = [];
...
function updateFile(filePath) {
  ...
  if (obj["t-s-1-8-4"] && obj["t-s-1-8-4"].test) {
    obj["t-s-1-8-4"].test = questions;
    const jsonStr = JSON.stringify(obj, null, 2);
    const newContent = `export const ${match[1]} = ${jsonStr};\n`;
    fs.writeFileSync(filePath, newContent, 'utf8');
    console.log("Updated " + filePath);
  } else {
    console.log("t-s-1-8-4 not found in " + filePath);
  }
}
