...
async function main() {
  ...
  // 2. Delete existing Added Question topics in database (cascade deletes their MCQs)
  console.log('🧹 Deleting old added question topics in database...');
  const { data: deletedTopics, error: delError } = await supabaseAdmin
    .from('topics')
    .delete()
    .eq('subject_id', subjectId)
    .ilike('title', 'Added Question %')
    .select('id, title');
  ...
  // 4. Distribute and prepare MCQ records to insert (exactly 40 questions per topic)
  const records = [];
  ...
  console.log('🎉 Database sync (40 per topic) completed successfully!');
}
