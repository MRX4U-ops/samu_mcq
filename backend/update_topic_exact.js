...
async function run() {
  ...
  if (args.length !== 2) {
    console.error('Usage: node update_topic_exact.js <TopicNumber> <Filename>');
    process.exit(1);
  }
  ...
  const lines = text.split('\n').map(l => l.trim()).filter(l => 
    l && 
    !l.startsWith('Test Questions') && 
    !l.startsWith('___') && 
    l !== 'Medical biology with elements of ecology'
  );
  const mcqsToInsert = [];
  ...
  console.log(`Found ${mcqsToInsert.length} test questions for Topic ${topicNum}.`);

  console.log(`Cleaning existing test_question for Topic ${topicNum}...`);
