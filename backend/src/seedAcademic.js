...
const seedAcademicStructure = async () => {
  ...
  try {
    ...
    for (const data of academicData) {
      const course = await Course.create({ title: data.course });
      console.log(`Injecting Course: ${course.title}`);

      for (const subjectTitle of data.subjects) {
        const subject = await Subject.create({
          courseId: course._id,
          title: subjectTitle
        });

        // Generate Topic 1 to Topic 15
        for (let i = 1; i <= 15; i++) {
          await Topic.create({
    ...
    console.log("✅ ACADEMIC DATABASE UPDATED WITH TOPICS 1-15.");
    process.exit();
  } catch (error) {
