...
const blocks = rawText.split(/Question\s+\d+/).filter(b => b.trim());
const questions = [];
...
function updateFile(filePath) {
  ...
  if (obj["t-s-1-8-5"] && obj["t-s-1-8-5"].test) {
    obj["t-s-1-8-5"].test = questions;
    const jsonStr = JSON.stringify(obj, null, 2);
    const newContent = `export const ${match[1]} = ${jsonStr};\n`;
    fs.writeFileSync(filePath, newContent, 'utf8');
    console.log("Updated " + filePath);
  } else {
    console.log("t-s-1-8-5 not found in " + filePath);
  }
}
